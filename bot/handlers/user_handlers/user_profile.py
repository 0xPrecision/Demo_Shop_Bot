...
async def show_profile_orders_menu(callback: CallbackQuery, t, state: FSMContext, **_):
	...
	"""
    await delete_request_and_user_message(callback.message, state)
    await callback.message.answer(t('user_profile.messages.ya-hochu-posmotret'), reply_markup=profile_orders_keyboard())
    await callback.answer()
...
async def show_profile_data(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    if user_profile and is_profile_complete(user_profile):
        ...
        await callback.message.edit_text(t('user_profile.messages.vash-profil-otsutstvuet-hotite'), reply_markup=create_profile())
...
async def confirm_profile(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    if cur_state == ProfileStates.create_profile:
        await callback.message.answer(t('user_profile.messages.vash-profil-uspeshno-sozdan'), reply_markup=cart_back_menu())
        ...
        await callback.message.answer(t('user_profile.messages.vash-profil-uspeshno-obnovlen'), reply_markup=cart_back_menu())
        await state.clear()
    await callback.answer()
...
async def edit_data_handler(callback: CallbackQuery, t, state: FSMContext, **_):
	...
	"""
    await delete_request_and_user_message(callback.message, state)
    edit_msg = await callback.message.answer(t('user_checkout.messages.chto-vy-hotite-izmenit'), reply_markup=edit_profile_keyboard())
