...
async def show_profile_orders_menu(callback: CallbackQuery, t, state: FSMContext, **_):
	...
	"""
    await delete_request_and_user_message(callback.message, state)
    await callback.message.answer(t('user_profile.messages.ya-hochu-posmotret'), reply_markup=profile_orders_keyboard(t))
    await callback.answer()
...
async def show_profile_data(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    if user_profile and is_profile_complete(user_profile):
        ...
        await callback.message.edit_text(t('user_profile.messages.vash-profil-otsutstvuet-hotite'), reply_markup=create_profile(t))
...
async def confirm_profile(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    if cur_state == ProfileStates.create_profile:
        await callback.message.answer(t('user_profile.messages.vash-profil-uspeshno-sozdan'), reply_markup=cart_back_menu(t))
    elif cur_state == ProfileStates.confirm:
        await update_user_profile(user_id, name=data.get('name'), phone=data.get('phone'), address=data.get('address'))
        await callback.message.answer(t('user_profile.messages.vash-profil-uspeshno-obnovlen'), reply_markup=cart_back_menu(t))
        await state.clear()
    await callback.answer()
...
async def edit_data_handler(callback: CallbackQuery, t, state: FSMContext, **_):
	...
	"""
    await delete_request_and_user_message(callback.message, state)
    edit_msg = await callback.message.answer(t('user_checkout.messages.chto-vy-hotite-izmenit'), reply_markup=edit_profile_keyboard(t))
...
async def edit_profile_name_callback(callback: CallbackQuery, t, state: FSMContext, **_):
    """
...
async def edit_profile_phone_callback(callback: CallbackQuery, t, state: FSMContext, **_):
    """
...
async def edit_profile_address_callback(callback: CallbackQuery, t, state: FSMContext, **_):
    """
