...
from bot.keyboards.user.user_catalog_keyboards import (
    products_keyboard,
    show_categories_keyboard,
...
async def show_categories(callback: CallbackQuery, t, **_) -> None:
    ...
    if not categories:
        await callback.message.answer(
            t("user_catalog.messages.kategorii-ne-najdeny"), reply_markup=main_menu(t)
        )
        return
    await callback.message.answer(
        t("user_catalog.messages.vyberite-kategoriyu"),
        reply_markup=show_categories_keyboard(categories, t),
    )
...
async def show_products_in_category(
    ...
    category_products = [
        p for p in all_products if p.category and p.category.name == category_name
    ]
    if not category_products:
        await callback.message.edit_text(
            t("user_catalog.messages.v-etoj-kategorii-poka"), reply_markup=main_menu(t)
        )
        await callback.answer()
        return
    page_products, total_pages, current_page = paginate(
        category_products, page, PAGE_SIZE
    )
    text = t("category.header").format(category_name=category_name)
    markup = products_keyboard(
        page_products, category_name, current_page, total_pages, t
    )
...
async def show_product_info(callback: CallbackQuery, t, **_):
    ...
    caption = t("product.card.caption").format(
        name=product.name,
