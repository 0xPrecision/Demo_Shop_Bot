...
from bot.keyboards.user.user_checkout_keyboards import (
    change_address_keyboard,
    checkout_edit_keyboard,
    delivery_methods_keyboard,
    payment_methods_keyboard,
    profile_data_confirm_keyboard,
)
...
from bot.utils.user_utils.user_checkout_utils import (
    editing_address,
    editing_comment,
    editing_delivery,
    editing_name,
...
async def place_an_order_handler(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    if not cart_items:
        await callback.message.answer(
            t("user_cart.messages.vasha-korzina-pusta"), reply_markup=cart_back_menu(t)
        )
        return
    ...
    if user_profile and all(
        (
            getattr(user_profile, field) not in ("", None, "-")
            for field in fields_to_check
        )
    ):
        text = t("user_checkout.misc.v-vashem-profile-fio").format(
            full_name=user_profile.full_name,
            phone=user_profile.phone,
            address=user_profile.address,
        )
        await callback.message.answer(
            text, reply_markup=profile_data_confirm_keyboard(t)
        )
...
async def use_profile_choice_handler(
    ...
    if callback.data == "use_profile":
        ...
        comment = await callback.message.answer(
            t("user_checkout.messages.vvedite-kommentarij-k-zakazu"),
...
async def order_comment_handler(message: Message, state: FSMContext, t):
    ...
    text = t("user_checkout.misc.zapolnite-dannye-dlya-zakaza").format(
        full_name=data.get("name"), phone=data.get("phone"), comment=data.get("comment")
    )
...
async def choose_payment_method(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    method = {
        ...
        "pay_cash": t("user_checkout_keyboards.buttons.nalichnye"),
        "pay_yoomoney": t("user_checkout_keyboards.buttons.yumoney"),
    }[callback.data]
    if not method:
        await callback.answer(
        ...
        return
    if method in (
        t("user_checkout_keyboards.buttons.kartoj-onlajn"),
        t("user_checkout_keyboards.buttons.yumoney"),
    ):
        try:
            await callback.message.answer(
                text=t("user_checkout.messages.etot-sposob-oplaty").format(
                    payment=method
                ),
                reply_markup=payment_methods_keyboard(t),
            )
    ...
    data = await state.get_data()
    text = t("user_checkout.misc.zapolnite-dannye-dlya-zakaza.2").format(
    ...
    await send_step_and_cleanup(
        ...
        text=text,
        state=state,
        reply_markup=delivery_methods_keyboard(t),
    )
...
async def choose_delivery_method(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    """
    method = {
        "delivery_courier": t("user_checkout.dostavka-kurer"),
        "delivery_pickup": t("user_checkout_keyboards.buttons.samovyvoz"),
    }[callback.data]
    ...
    if method == t("user_checkout.dostavka-kurer"):
        ...
        if user_profile.address not in ("", "-", None):
            cur_address = (
                ...
                if user_profile and hasattr(user_profile, "address")
                else t("no-address")
            )
            msg = await callback.message.answer(
                t("user_checkout.address.prompt").format(current_address=cur_address),
                reply_markup=change_address_keyboard(t),
            )
            await state.update_data(main_message_id=msg.message_id)
            await state.set_state(OrderStates.choose_address_option)
        else:
            msg = await callback.message.answer(
                t("user_checkout.messages.vvedite-adres-dostavki"),
                reply_markup=cart_back_menu(t),
            )
            await state.update_data(main_message_id=msg.message_id)
...
async def choose_address_option_handler(
    callback: CallbackQuery, t, state: FSMContext, **_
):
    """
...
async def edit_delivery_callback(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    await delete_request_and_user_message(callback.message, state)
    msg = await callback.message.answer(
        t("user_checkout.messages.vyberite-sposob-dostavki"),
        reply_markup=delivery_methods_keyboard(t),
    )
    await state.update_data(main_message_id=msg.message_id)
...
async def order_confirm_handler(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    order = await create_order(
        ...
        name=data.get("name"),
        phone=data.get("phone"),
        status=t("order.status.in_progress"),
