...
async def change_order_status_menu(callback: CallbackQuery, state: FSMContext, t, **_):
    ...
    if not order:
        await callback.answer(t('admin_orders.messages.zakaz-ne-najden'), show_alert=True)
        return
    await message.edit_reply_markup(reply_markup=status_keyboard(order.id, t, order.status))
    await callback.answer()
...
async def set_order_status(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    try:
        text = t('admin_orders.misc.soobschenie-dlya-polzovatelya-vash').format(id=order.id, status_label=status_label)
