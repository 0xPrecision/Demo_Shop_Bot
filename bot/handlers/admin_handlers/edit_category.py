...
from ...keyboards.admin.catalog_keyboards import (
    back_menu,
    confirm_deletion_category,
    edit_or_deletion_category,
)
...
async def choose_category_to_edit(callback: CallbackQuery, t, **_):
    ...
    await callback.message.edit_text(
        t("admin_catalog.messages.vyberete-dejstvie"),
        reply_markup=edit_or_deletion_category(category_id, t),
    )
...
async def rename_category_start(callback: CallbackQuery, t, state: FSMContext, **_):
    ...
    msg = await callback.message.edit_text(
        t("edit_category.messages.vvedite-novoe-nazvanie-dlya")
    )
...
async def rename_category_process(message: Message, t, state: FSMContext, **_):
    ...
    if not new_name:
        msg = await message.answer(
            t("edit_category.messages.nazvanie-ne-mozhet-byt"),
    ...
    if exists:
        msg = await message.answer(
            t("edit_category.messages.kategoriya-s-takim-imenem"),
            reply_markup=back_menu(t),
        )
        await state.update_data(main_message_id=msg.message_id)
        return
    ...
    msg = await message.answer(
        t("edit_category.rename-category").format(new_name=new_name),
        reply_markup=back_menu(t),
    )
    await state.update_data(main_message_id=msg.message_id)
    await state.clear()
...
async def delete_category_confirm(callback: CallbackQuery, t, **_):
    ...
    if count > 0:
        await callback.answer(
            t("edit_category.messages.v-kategorii-est-tovary"), show_alert=True
        )
        return
    category = await Category.get(id=cat_id)
    await callback.message.edit_text(
        t("edit_category.you-wanna-delete").format(category_name=category.name),
        reply_markup=confirm_deletion_category(cat_id, t),
    )
    await callback.answer()
...
async def delete_category_execute(callback: CallbackQuery, t, **_):
    ...
    await callback.message.edit_text(
        t("edit_category.messages.kategoriya-udalena"), reply_markup=back_menu(t)
    )
    await callback.answer()
