...
@admin_only
async def start_add_category(callback: CallbackQuery, t, state: FSMContext, **_):
    """
    ...
    await state.update_data(draft_state=cur_state, draft_data=cur_data)
    msg = await callback.message.edit_text(t('add_category.messages.vvedite-nazvanie-novoj-kategorii'), reply_markup=back_menu(t))
...
@admin_only
async def add_category_name(message: Message, t, state: FSMContext, **_):
    """
