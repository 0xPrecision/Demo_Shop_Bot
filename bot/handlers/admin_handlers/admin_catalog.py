...
from bot.keyboards.admin.catalog_keyboards import (
    ...
    show_products_or_edit_category,
)
...
from ...utils.admin_utils.catalog_utils import (
    filter_or_change_pr_category,
    get_products_info,
)
...
async def admin_catalog_menu(callback: CallbackQuery, t, **_):
    ...
    await callback.message.edit_text(
        t("admin_catalog.messages.chto-vy-hotite-prosmotret-redaktirovat"),
...
async def admin_products_by_category(callback: CallbackQuery, t, **_):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=show_products_or_edit_category(category_id, t),
    )


@router.callback_query(F.data.startswith("admin_category_filter:"))
@admin_only
async def admin_products_in_category(callback: CallbackQuery, state: FSMContext, t):
    """
