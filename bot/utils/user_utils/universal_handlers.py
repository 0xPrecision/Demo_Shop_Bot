...
async def universal_phone_handler(message: Message, state: FSMContext, t) -> None:
    ...
    if state_name == OrderStates.waiting_for_phone.state:
        text = t("universal_handlers.misc.zapolnite-dannye-fio.2").format(
            name=name, phone=phone
        )
        await send_step_and_cleanup(
            message, text, state, reply_markup=cart_back_menu(t)
        )
        await state.set_state(OrderStates.waiting_for_comment)
    elif state_name == ProfileStates.waiting_for_phone.state:
        text = t("universal_handlers.misc.zapolnite-dannye-profilya").format(
            name=name, phone=phone
        )
        await send_step_and_cleanup(
            message, text, state, reply_markup=cart_back_menu(t)
