...
async def start_manual_checkout(message_or_callback, state: FSMContext, t, **_):
    """
    ...
    main_msg = await target_message.answer(t('user_common_utils.messages.zapolnite-dannye-1-fio'), reply_markup=cart_back_menu(t))
...
async def validation_process_name(message: Message, state: FSMContext, t, **_):
	...
	"""
    warn = await message.answer(t('user_common_utils.messages.vvedite-korrektnoe-imya-tolko'), reply_markup=cart_back_menu(t))
    await state.update_data(main_message_id=warn.message_id)

async def validation_process_phone(message: Message, state: FSMContext, t, **_):
    """
    Shows a warning about an invalid phone number.
    :param message: User message.
    :param state: FSMContext for storing main_message_id.
	"""
    warn = await message.answer(t('user_common_utils.messages.vvedite-korrektnyj-telefon-tolko'), reply_markup=cart_back_menu(t))
    await state.update_data(main_message_id=warn.message_id)

async def validation_process_address(message: Message, state: FSMContext, t, **_):
    """
    Shows a warning about an invalid address.
    :param message: User message.
    :param state: FSMContext for storing main_message_id.
	"""
    warn = await message.answer(t('user_common_utils.messages.vvedite-korrektnyj-adres-primer'), reply_markup=cart_back_menu(t))
    await state.update_data(main_message_id=warn.message_id)
