...
from bot.keyboards.admin.catalog_keyboards import products_list_keyboard, admin_categories_keyboard, ask_of_create_product, ask_of_create_category, change_category_keyboard
...
async def get_products_info(callback: CallbackQuery, t, page: int, text: str, func: Awaitable[Tuple[List['Product'], bool, bool]], state: FSMContext, **_) -> None:
    ...
    if not products:
        ...
        return
    msg = await callback.message.answer(text=text, reply_markup=products_list_keyboard(products_for_kb, page, has_next, has_prev, t))
    await state.update_data(main_message_id=msg.message_id)

async def filter_or_change_pr_category(event: Union[CallbackQuery, Message], state: FSMContext, t, text: str=None, product_id: int=None, **_) -> None:
    """
    ...
    kb = change_category_keyboard(categories) if cur_state else admin_categories_keyboard(categories, t, **_)
